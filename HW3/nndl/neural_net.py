...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    f = lambda x: x*(x>0)
    h = f(X.dot(W1.T)+b1)
    # dont apply ReLU to output z
    scores = h.dot(W2.T)+b2
    ...
    # creating indices [1,y1],[2,y2]...[N,yN]
    a_ind = [np.arange(N),y]
    ...
    # dLdz = softmax_loss
    dLdz = smax - ind #N,C
    grads['W2'] = dLdz.T.dot(h)/N + reg*W2
    grads['b2'] = 1/N*np.sum(dLdz,axis=0)

    dLdh = dLdz.dot(W2)
    a = X.dot(W1.T)+b1
    # for relu backprop I(a>0)*dLdh
    dLda = (a>0)*dLdh
    grads['b1'] = 1/N*(np.sum(dLda,axis =0))
    grads['W1'] = 1/N*dLda.T.dot(X) + reg*W1
